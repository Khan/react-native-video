...
@implementation RCTVideo
{
	...
	bool _pendingSeek;
	NSDictionary * _Nullable _pendingSeekInfo;
	float _lastSeekTime;
	BOOL _reportProgressWhileSeeking;
...
- (instancetype)initWithEventDispatcher:(RCTEventDispatcher *)eventDispatcher
{
	if ((self = [super init])) {
		...
		_pendingSeek = false;
		_pendingSeekInfo = NULL;
		_lastSeekTime = 0.0f;
		_reportProgressWhileSeeking = true;
...
- (void)sendProgressUpdate
{
	...
	if (!_reportProgressWhileSeeking && _pendingSeek) {
		return;
	}
...
{
	_pendingSeekInfo = info;
	[self processSeek];
}

- (void)processSeek {
	if (_pendingSeekInfo == NULL) {
		_pendingSeek = false;
		return;
	}
	...
	NSNumber *seekTime = _pendingSeekInfo[@"time"];
	NSNumber *seekTolerance = _pendingSeekInfo[@"tolerance"];
	float seekTimeFloat = [seekTime floatValue];
	float seekToleranceFloat = [seekTolerance floatValue];
	_pendingSeekInfo = NULL;
	_pendingSeek = true;
	...
	if (item && item.status == AVPlayerItemStatusReadyToPlay) {
		...
		CMTime cmSeekTime = CMTimeMakeWithSeconds(seekTimeFloat, timeScale);
		CMTime current = item.currentTime;
		// TODO figure out a good tolerance level
		CMTime tolerance = CMTimeMake(seekToleranceFloat, timeScale);
		...
		// TODO(abdul): probably want to throw an error here.
		_pendingSeek = false;
	}
}
...
- (void)setReportProgressWhileSeeking:(BOOL)reportProgressWhileSeeking
{
	if (_reportProgressWhileSeeking != reportProgressWhileSeeking) {
		_reportProgressWhileSeeking = reportProgressWhileSeeking;
	}
}
